...
import { ChevronDown, Search, X, BookOpen, Target, BarChart2, Check, AlertCircle, Home, FileText, BrainCircuit, Languages } from 'lucide-react';

// Firebase SDK imports (실제 연동 시 주석 해제)
// import { initializeApp } from 'firebase/app';
// import { getFirestore, collection, getDocs, query, where } from 'firebase/firestore';
// import { getAuth, signInAnonymously, onAuthStateChanged, signInWithCustomToken } from 'firebase/auth';

// --- 데이터베이스 (Firestore 연동 시뮬레이션) ---
const allQuestions = [
  ...
  {
      id: "2016-21", year: 2016, questionNumber: 21,
      questionText: "[Item 21 was not scored.]",
      answerOptions: [],
      explanation: {
          ko: { concept: "채점되지 않은 문항", steps: [], distractors: [], summary: "이 문항은 공식 AP 시험에서 채점되지 않았습니다." },
          en: { concept: "Unscored Item", steps: [], distractors: [], summary: "This item was not scored on the official AP Exam." }
      },
      tags: [],
      difficulty: '없음'
  },
...
const Histogram = ({ data }) => ( <ResponsiveContainer width="100%" height={250}><BarChart data={data.values.map((v, i) => ({ name: data.labels[i], value: v }))} margin={{ top: 5, right: 20, left: 10, bottom: 20 }}><CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" /><XAxis dataKey="name" tick={{ fontSize: 12 }}><Label value={data.xAxisLabel} offset={-15} position="insideBottom" /></XAxis><YAxis tick={{ fontSize: 12 }}><Label value={data.yAxisLabel} angle={-90} position="insideLeft" style={{ textAnchor: 'middle' }} /></YAxis><Tooltip content={<CustomTooltip />} cursor={{fill: 'rgba(79, 70, 229, 0.1)'}} /><Bar dataKey="value" fill="rgba(79, 70, 229, 0.8)" /></BarChart></ResponsiveContainer> );
const CustomBoxPlot = ({ data }) => {
    ...
    return (
        <div className="w-full p-4 space-y-8">
            {boxPlots.map((d) => (
                <div key={d.name}>
                    <p className="text-sm font-semibold text-slate-600 mb-2">{d.name}</p>
                    <div className="relative h-12">
                        {/* Whisker line */}
                        <div className="absolute top-1/2 -translate-y-1/2 h-px bg-slate-400" style={{ left: `${scale(d.min)}%`, width: `${scale(d.max) - scale(d.min)}%` }}></div>
                        {/* Min cap */}
                        <div className="absolute top-1/2 -translate-y-1/2 h-6 w-px bg-slate-700" style={{ left: `${scale(d.min)}%` }}></div>
                        {/* Max cap */}
                        <div className="absolute top-1/2 -translate-y-1/2 h-6 w-px bg-slate-700" style={{ left: `${scale(d.max)}%` }}></div>
                        {/* Box */}
                        <div className="absolute top-1/2 -translate-y-1/2 h-10 bg-indigo-100 border-2 border-indigo-400" style={{ left: `${scale(d.q1)}%`, width: `${scale(d.q3) - scale(d.q1)}%` }}></div>
                        {/* Median line */}
                        <div className="absolute top-1/2 -translate-y-1/2 h-10 w-px bg-indigo-600" style={{ left: `${scale(d.median)}%` }}></div>
                    </div>
                </div>
            ))}
            <div className="relative h-px bg-slate-300 mt-10">
                 {/* Axis Ticks */}
                 {Array.from({length: Math.floor((maxVal - minVal) / 25) + 2}).map((_, i) => {
                    ...
                    return (
                        <div key={val} className="absolute -bottom-5" style={{ left: `${scale(val)}%`, transform: 'translateX(-50%)' }}>
...
const MultiDotPlot = ({ data }) => ( <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">{data.map(plot => ( <div key={plot.id} className="text-center border p-2 rounded-lg"><h4 className="font-semibold text-sm mb-2">{plot.id}</h4><div className="flex justify-around items-end h-24">{Object.entries(plot.data).map(([value, count]) => ( <div key={value} className="flex flex-col-reverse items-center w-6"><div className="flex flex-col-reverse">{Array.from({ length: count }).map((_, i) => ( <div key={i} className="w-2 h-2 bg-slate-700 rounded-full my-0.5"></div> ))}</div><span className="text-xs mt-1">{value}</span></div> ))}</div></div> ))}</div> );

const StemPlot = ({ data }) => {
    const allStems = [...Object.keys(data.female), ...Object.keys(data.male)].map(Number);
    const stems = Array.from(new Set(allStems)).sort((a, b) => b - a);

    return (
        <div className="font-mono text-sm bg-slate-50 p-4 rounded-lg">
            <div className="grid grid-cols-[1fr_auto_1fr] text-center gap-x-4">
                <div className="font-bold text-slate-700 border-b pb-1">Female</div>
                <div></div>
                <div className="font-bold text-slate-700 border-b pb-1">Male</div>
                {stems.map(stem => (
                    <React.Fragment key={stem}>
                        <div className="text-right py-1">{(data.female[stem] || '').split('').reverse().join(' ')}</div>
                        <div className="text-center font-bold text-slate-500 border-l border-r py-1">{stem}</div>
                        <div className="text-left py-1">{(data.male[stem] || '').split('').join(' ')}</div>
                    </React.Fragment>
                ))}
            </div>
            <div className="text-center mt-4 text-xs text-slate-500 font-semibold">{data.key}</div>
        </div>
    );
};
...
const ScatterPlot = ({ data }) => (
    <ResponsiveContainer width="100%" height={300}>
        <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <Scatter name="Defective Parts" data={data.points} fill="rgba(79, 70, 229, 0.8)" />
        </ScatterChart>
    </ResponsiveContainer>
);
...
const SegmentedBarChart = ({ data }) => (
    <ResponsiveContainer width="100%" height={200}>
        <BarChart layout="vertical" data={data} stackOffset="expand" margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
            ...
            <Legend />
            <Bar dataKey="Activity A" stackId="a" fill="#4f46e5" name="Activity A" />
            <Bar dataKey="Activity B" stackId="a" fill="#818cf8" name="Activity B" />
            <Bar dataKey="Activity C" stackId="a" fill="#c7d2fe" name="Activity C" />
        </BarChart>
    </ResponsiveContainer>
);
    ...
    return (
        <div>
            {parts.map((part, index) => {
                ...
                const isRomanList = lines.some(line => /^\s*(I|V|X)\.\s/.test(line));

                if (isRomanList) {
                    return (
                        <div key={index} className="my-2 space-y-2">
                            {lines.map((item, itemIndex) => (
                                <p key={itemIndex} className="pl-4">{item}</p>
                            ))}
                        </div>
                    );
                }
                ...
                if (part.includes(' = ') || part.includes(' ± ')) {
                     return <div key={index} className="my-2 p-3 bg-slate-100 rounded-md text-center font-mono tracking-wider">{part}</div>
                }
...
const PracticeSession = ({ questions, onBack, title }) => {
    ...
    const renderChart = (question) => {
        switch (question.chartType) {
            case 'Histogram': return <Histogram data={question.chartData} />;
            case 'BoxPlot': return <CustomBoxPlot data={question.chartData} />;
            case 'MultiDotPlot': return <MultiDotPlot data={question.chartData} />;
            case 'StemPlot': return <StemPlot data={question.chartData} />;
            case 'ScatterPlot': return <ScatterPlot data={question.chartData} />;
            case 'SegmentedBarChart': return <SegmentedBarChart data={question.chartData} />;
            default: return null;
        }
    };
...
export default function App() {
    ...
    if(isLoading) {
        return <div className="min-h-screen flex items-center justify-center"><p>AP Stats Lab 불러오는 중...</p></div>
    }
            ...
            <div className="w-full max-w-7xl mx-auto">
                <header className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3 cursor-pointer" onClick={() => setPage('dashboard')}>
                        ...
                        <h1 className="text-3xl font-bold text-slate-800">Wonjae's AP Lab</h1>
                    </div>
